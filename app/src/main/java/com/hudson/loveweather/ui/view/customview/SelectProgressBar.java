...
public class SelectProgressBar extends View {
    ...
    private static final int DEFAULT_WARNING_COLOR = UIUtils.getColor(R.color.warning_color);
    ...
    private int mWarningValueStart = -1;//警告值
    private int mWarningValueEnd = -1;
...
//        System.out.println("文字宽度"+mPaint.measureText(mMaxValue+""));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(mSelectedValue>=mWarningValueStart&&mSelectedValue<=mWarningValueEnd){
            mCirclePaint.setColor(DEFAULT_WARNING_COLOR);
            mIsWarningState = true;
        }else{
            mCirclePaint.setColor(mCircleColor);
            mIsWarningState = false;
        }
